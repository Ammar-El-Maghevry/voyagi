...
export class PassengerBookingsController {
  ...
  @RateLimit('booking')
  @ApiOperation({
    summary: 'Create an authenticated passenger-owned seat hold.',
  })
  @ApiHeader({ name: 'Idempotency-Key', required: true })
  @ApiCreatedResponse({ type: BookingResponseDto })
  @ApiConflictResponse({
    description: 'Seat or idempotency conflict, or trip not bookable.',
  })
  @ApiBadRequestResponse({ description: 'Invalid body or Idempotency-Key.' })
  @ApiUnprocessableEntityResponse({
    description: 'Passenger or seat selection is invalid.',
  })
  async create(
  ...
  @Get()
  @ApiOperation({
    summary: 'List only the authenticated passenger owner’s online bookings.',
  })
  @ApiOkResponse({
  ...
  @ApiOperation({
    summary: 'Read one authenticated passenger-owned online booking.',
  })
  ...
  async get(
    ...
    return BookingResponseDto.from(
      await this.getBooking.owned(principal.userId, bookingId),
    );
  }
  ...
  @ApiConflictResponse({
    description: 'The booking is terminal or not cancellable.',
  })
  @ApiNotFoundResponse({
    description: 'No passenger-owned online booking with this id.',
  })
  async cancel(
    @CurrentPrincipal() principal: AuthenticatedPrincipal,
    @Param('bookingId') bookingId: string,
    ...
    return BookingResponseDto.from(
      await this.cancelBooking.owned(principal.userId, bookingId),
    );
  }
  ...
  @ApiOperation({
    summary: 'List sanitized events for a passenger-owned booking.',
  })
  @ApiNotFoundResponse({
    description: 'No passenger-owned online booking with this id.',
  })
  async events(
    ...
    const pagination = resolvePagination(query);
    const page = await this.listEvents.owned(
      principal.userId,
      bookingId,
      pagination,
    );
...
export class CompanyBookingsController {
  ...
  @ApiBadRequestResponse({ description: 'Invalid body or Idempotency-Key.' })
  @ApiConflictResponse({
    description: 'Seat, idempotency, or trip-bookability conflict.',
  })
  @ApiUnprocessableEntityResponse({
    description: 'Passenger or seat selection is invalid.',
  })
  ...
  @ApiOperation({
    summary: 'List company bookings within the caller’s entitlement scope.',
  })
  @ApiOkResponse({
    description: 'Standard paginated collection envelope.',
    schema: {
  ...
  async list(
    ...
    const pagination = resolvePagination(query);
    const page = await this.listBookings.company(
      principal.userId,
      companyId,
      pagination,
    );
  ...
  @ApiOkResponse({ type: BookingResponseDto })
  @ApiOperation({
    summary: 'Read one company booking within entitlement scope.',
  })
  ...
  @ApiOkResponse({ type: BookingResponseDto })
  @ApiOperation({
    summary: 'Cancel an unpaid booking within branch entitlement scope.',
  })
  @ApiConflictResponse({
    description: 'The booking is terminal or not cancellable.',
  })
  ...
  @ApiOkResponse(bookingEventCollectionResponse)
  @ApiOperation({
    summary: 'List sanitized company booking events within entitlement scope.',
  })
  @ApiNotFoundResponse({ description: 'The scoped booking is not visible.' })
  async events(
    @CurrentPrincipal() principal: AuthenticatedPrincipal,
